...
async def read_excel():
    ...
    try:
        for row in rows:
            print(row, end="\n")
            print(row[0])
            # await db.add(User(username=str(row[1]), password=str(row[2]), external_id=str(row[0]), full_name='Akrom Amirov', gender='Erkak',
            #                   faculty_id=(row[6]), turniket_id=str(row[7]), begin_time=(row[8]), end_time=(row[9]),
            #                   image_url='https://hemis.tdpu.uz/static//pi/b/4/b46a147cd59b84df2eafb2aceee64724.jpg', created_at=row[11], student_id='20251212', file_path=str(row[13]),
            #                   group_id=row[14], group_name=str(row[15]), speciality_id=str(row[16]), speciality_name=str(row[17])
            #                   ))
            break
